...
public:
	...
	void run(int iNumOfThreads, int iHappendID)
	{
		...
		else
		{
			...
			int* affect = new int[iNumOfThreads];
			int* ready = new int[iNumOfThreads];
			/*memset(affect, 0, sizeof(int) * iNumOfThreads);
			memset(ready, 0, sizeof(int) * iNumOfThreads);*/
			vector<int> seperator;
			balanceGroup(vBlockList.size(), iNumOfThreads, seperator);
			for (int i = 0; i < iNumOfThreads; i++)
				vThreads.push_back(thread(&BlockList::handle, this, seperator[i], seperator[i+1], iHappendID, ref(affect[i]), ref(ready[i])));
			for (int i = 0; i < iNumOfThreads; i++)
				vThreads[i].join();
			for (int i = 0; i < iNumOfThreads; i++)
			{
				iCountAffect += affect[i];
				iCountReady += ready[i];
			}
			//end of your code
		}
...
{
	string myStudentID("22330034");//replace by yours.
	int iNumOfThreads = 2;//change it to another value for test
